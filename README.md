...
Перед публичной публикацией владелец должен отозвать credential, который ранее
находился в локальном шаблоне окружения. Его значение не сохранено в проекте.

## Лицензия

[MIT](LICENSE). До отдельного решения владельца проект не публикуется и не
отправляется в удалённый репозиторий.
