...
import { reviewModule1BetaVocabulary, syncModule1BetaVocabulary, type Module1BetaVocabularyItem, type Module1BetaVocabularySeed } from "../lib/api";

export function contentVocabulary(completedLessonSlugs: string[]): Module1BetaVocabularySeed[] {
  const completed = new Set(filterKnownLessonSlugs(a1CourseModules, completedLessonSlugs));
  const result: Module1BetaVocabularySeed[] = [];
...
export function Module1BetaVocabulary({ completedLessonSlugs }: { completedLessonSlugs: string[] }) {
  const [items, setItems] = useState<Module1BetaVocabularyItem[]>([]);
  const [filter, setFilter] = useState("");
  const [loading, setLoading] = useState(true);
  const [reviewingId, setReviewingId] = useState<number | null>(null);
  const [error, setError] = useState("");
  useEffect(() => { void syncModule1BetaVocabulary(contentVocabulary(completedLessonSlugs)).then((stored) => setItems(stored.filter((item) => completedLessonSlugs.includes(item.lesson_slug)))).catch((cause) => setError(cause instanceof Error ? cause.message : "Не удалось загрузить слова.")).finally(() => setLoading(false)); }, [completedLessonSlugs]);
  ...
  return <section className="module-beta-exercises module-beta-vocabulary" aria-labelledby="module-beta-vocabulary-title"><div className="module-beta-section-heading"><div><span>Словарь новой версии</span><h3 id="module-beta-vocabulary-title">Слова</h3></div><p>Слова открываются постепенно после завершения каждой темы.</p></div><div className="module-beta-vocabulary-actions"><button type="button" onClick={download} disabled={!visible.length}>Скачать .txt</button></div>{error && <p className="module-beta-persistence-error" role="alert">{error}</p>}{loading ? <p>Загружаю словарь…</p> : !completedLessonSlugs.length ? <p className="module-beta-empty">Завершите первую тему — её слова появятся здесь.</p> : <>{due.length > 0 && <section className="module-beta-vocabulary-due"><div><strong>Время повторить</strong><span>{due.length} элементов</span></div><article><strong lang="sk">{due[0].word}</strong><span>{due[0].translation}</span>{due[0].example && <small lang="sk">{due[0].example}</small>}<button type="button" onClick={() => void review(due[0])} disabled={reviewingId !== null}>{reviewingId === due[0].id ? "Сохраняю…" : "Повторил"}</button></article></section>}<div className="module-beta-vocabulary-tags"><button type="button" className={!filter ? "active" : ""} onClick={() => setFilter("")}>Все</button>{topics.map(([slug, title]) => <button type="button" className={filter === slug ? "active" : ""} key={slug} onClick={() => setFilter(slug)}>{title}</button>)}</div><div className="module-beta-vocabulary-list">{visible.map((item) => <article key={item.id}><div><strong><span>{item.translation}</span> — <span lang="sk">{item.word}</span></strong>{item.example && <small lang="sk">{item.example}</small>}</div><span>{item.review_count === 0 ? "Новое" : item.is_due ? "К повторению" : `Через ${item.interval_days} дн.`}</span></article>)}</div></>}</section>;
}
