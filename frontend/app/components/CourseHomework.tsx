...
import { deleteCourseHomework, generateCourseHomework, getCourseHomework, submitCourseHomework, type CourseHomework as Homework, type CourseHomeworkAttempt } from "../lib/api";
...
export function CourseHomework({ completedLessonSlugs, mistakeHints }: { completedLessonSlugs: string[]; mistakeHints: Array<{ lessonSlug: string; text: string }> }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const item = mode === "topic"
        ? await generateCourseHomework({ lesson_slug: lesson.slug, lesson_title: lesson.title, theory: theoryFor(lesson.slug), known_mistakes: mistakeHints.filter((hint) => hint.lessonSlug === lesson.slug).map((hint) => hint.text) })
        : await generateCourseHomework({ lesson_slug: "course-progress", lesson_title: "Общий прогресс Slovak A1", theory: completedLessons.map((item) => `Тема: ${item.title}\n${theoryFor(item.slug)}`).join("\n\n").slice(0, 12_000), known_mistakes: mistakeHints.filter((hint) => completedLessonSlugs.includes(hint.lessonSlug)).map((hint) => hint.text) });
  ...
  const submit = async (event: FormEvent) => {
    ...
    try { const checked = await submitCourseHomework(selected.id, answer.trim()); setResult(checked); setItems((current) => current.map((item) => item.id === selected.id ? { ...item, latest_attempt: checked } : item)); setAnswer(""); }
  ...
  return <section className="course-exercises course-homework" aria-labelledby="course-homework-title">
    <div className="course-section-heading"><div><span>Самостоятельная практика</span><h3 id="course-homework-title">Домашнее задание</h3></div><p>AI создаёт задание по теории выбранной темы и учитывает сохранённые ошибки.</p></div>
    <div className="course-reading-modes" role="group" aria-label="Режим генерации домашнего задания"><button type="button" className={mode === "topic" ? "active" : ""} onClick={() => { setMode("topic"); setSelectedId(null); setResult(null); setAnswer(""); }}>По теме</button><button type="button" className={mode === "progress" ? "active" : ""} onClick={() => { setMode("progress"); setSelectedId(null); setResult(null); setAnswer(""); }}>По общему прогрессу</button></div>
    <div className="course-exercise-toolbar">{mode === "topic" ? <label><span>Завершённая тема</span><select value={lessonSlug} disabled={!completedLessons.length} onChange={(event) => { setLessonSlug(event.target.value); setSelectedId(null); setResult(null); setAnswer(""); }}>{completedLessons.map((item) => <option value={item.slug} key={item.slug}>{item.title}</option>)}</select></label> : <div className="course-reading-scope"><span>Доступный материал</span><strong>{completedLessons.length} завершённых тем</strong><small>Ошибки учитываются только по пройденному материалу.</small></div>}<button type="button" onClick={() => void generate()} disabled={generating || !completedLessons.length}>{generating ? "Создаю…" : "Создать задание"}</button></div>
