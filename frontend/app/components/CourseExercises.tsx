...
export function CourseExercises({ completedLessonSlugs }: { completedLessonSlugs: string[] }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const item = mode === "topic"
        ? await generateCourseExercise({ lesson_slug: lesson.slug, lesson_title: lesson.title, theory: lessonTheory(lesson.slug) })
        : await generateCourseExercise({ lesson_slug: "course-progress", lesson_title: "Общий прогресс Slovak A1", theory: completedLessons.map((item) => `Тема: ${item.title}\n${lessonTheory(item.slug)}`).join("\n\n").slice(0, 12_000) });
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const result = await answerCourseExercise(selected.id, answer.trim());
  ...
  return <section className="course-exercises" aria-labelledby="course-exercises-title">
    <div className="course-section-heading"><div><span>Отдельная механика новой версии</span><h3 id="course-exercises-title">Упражнения</h3></div><p>Задания и ответы сохраняются в SQLite отдельно от старого приложения.</p></div>
    <div className="course-reading-modes" role="group" aria-label="Режим генерации упражнения"><button type="button" className={mode === "topic" ? "active" : ""} onClick={() => { setMode("topic"); setSelectedId(null); setAssessment(null); setAnswer(""); }}>По теме</button><button type="button" className={mode === "progress" ? "active" : ""} onClick={() => { setMode("progress"); setSelectedId(null); setAssessment(null); setAnswer(""); }}>По общему прогрессу</button></div>
    <div className="course-exercise-toolbar">
      {mode === "topic" ? <label><span>Завершённая тема</span><select value={lessonSlug} disabled={!completedLessons.length} onChange={(event) => { setLessonSlug(event.target.value); setSelectedId(null); setAssessment(null); setAnswer(""); }}>{completedLessons.map((item) => <option key={item.slug} value={item.slug}>{item.title}</option>)}</select></label> : <div className="course-reading-scope"><span>Доступный материал</span><strong>{completedLessons.length} завершённых тем</strong><small>Незавершённые темы в задание не попадут.</small></div>}
